...
**Bilan sur les 29 modules du cahier des charges (V4.0) : 26 modules complets et testés.** Les 3 restants (Paiements réels, Wallet, API partenaires) sont bloqués par des éléments externes — voir la section suivante.
...
| Gestion des chauffeurs (complétée) | Module 4 : permis détaillé (catégorie A à E, date d'expiration), certificats (ex. transport matières dangereuses), disponibilité en temps réel (disponible/en mission/indisponible — passe automatiquement "en mission" à l'acceptation, redevient "disponible" à la livraison, ou ajustable manuellement pour un congé), historique des missions par chauffeur, notation propre et distincte de celle du transporteur |
| Maintenance de flotte | Module 26 : journal d'interventions par véhicule (entretiens, vidanges, pneus, réparations — avec kilométrage et coût), échéances réglementaires suivies (contrôle technique, assurance — jamais de date inventée par défaut), bannière d'alerte sur les échéances proches (fenêtre de 30 jours) avec distinction claire entre échéance à venir et déjà dépassée, kilométrage du véhicule qui se met à jour automatiquement avec la valeur la plus récente |
...
| Transport collaboratif | Module 29 : le client peut accepter de partager son transport (jamais activé sans son accord) ; un algorithme déterministe et explicable (pas une IA) regroupe les demandes compatibles (même trajet, fenêtre de date de 12h, poids restant suffisant, marchandises compatibles — les catégories sensibles ne se mélangent jamais) ; répartition des coûts proportionnelle au poids avec une vraie économie mesurable (le forfait de base et le coût de distance ne sont payés qu'une fois pour tout le groupe) ; **confidentialité stricte respectée (anomalie #9 du cahier des charges, testée explicitement)** — seuls la ville de livraison et l'heure estimée des autres membres sont visibles, jamais leur nom, téléphone ou contenu du colis — **limite assumée** : pas de vrai moteur d'optimisation d'itinéraire (correspondance exacte des libellés de trajet, pas d'analyse géographique de trajets proches) |
